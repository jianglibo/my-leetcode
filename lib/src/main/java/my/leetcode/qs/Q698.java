...
public class Q698 {
	class Solution {
		private boolean backtrack(int[] arr, int index, int count, int currSum, int k,
			...
			// We made k - 1 subsets with target sum and last subset will also have
			// target sum.
			if (count == k - 1) {
				return true;
			}
			...
			for (int j = index; j < n; ++j) {
				if (!taken[j]) {
					...
					// If using current jth element in this subset leads to make
					// all valid subsets.
					if (backtrack(arr, j + 1, count, currSum + arr[j], k,
							targetSum, taken)) {
						return true;
					}
			...
			// We were not able to make a valid combination after picking each element
			// from the array,
	...
	// class Solution {
	// private boolean backtrack(int[] arr, int count, int currSum, int k, int targetSum,
	// boolean[] taken) {
	// int n = arr.length;
	...
	// // If using current jth element in this subset leads to make
	// // all valid subsets.
	// if (backtrack(arr, count, currSum + arr[j], k, targetSum,
	// taken)) {
	// return true;
	// }
